...
const FamilyButtonContainer: FC<FamilyButtonContainerProps> = ({
  ...
  return (
    <motion.div
      className={cn(
        "relative   border-border border shadow-card flex flex-col space-y-1  items-center text-foreground cursor-pointer z-10",
        !isExpanded
          ? "bg-card"
          : ""
      )}
    ...
    >
      ...
      >
        {isExpanded ? (
          <motion.div
            className="p-[10px] group bg-secondary border border-border hover:border-primary text-foreground rounded-full shadow-2xl transition-colors duration-300 "
          ...
          >
            <XIcon
              className={cn(
                "h-7 w-7 text-cyan-100/30 group-hover:text-neutral-500 transition-colors duration-200 "
              )}
            />
          </motion.div>
        ) : (
