  ...
  return (
    ...
    >
      <motion.div
        ...
        // className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-end h-14 p-2 gap-3 bg-neutral-50 bg-opacity-90 rounded-xl"
        className={cn(
          "absolute bottom-4 left-3/4 transform -translate-x-1/2 flex items-end h-14 p-2 gap-3  bg-opacity-90 rounded-xl",
          " bg-neutral-50 p-2 no-underline shadow-sm transition-colors hover:bg-neutral-100 ",
      ...
      <AnimatePresence mode="popLayout">
        {!animatingIndexes.includes(parseInt(id)) ? (
          <motion.img
            ...
            className="relative z-0 w-1/2 h-1/2 rounded-full border border-black/30"
            src={src}
            alt=""
          />
        ) : null}
  ...
  return (
    <div className="flex flex-col items-center gap-1" key={id}>
      ...
      <motion.button
        ...
        className="rounded-lg border aspect-square  border-black/5 border-opacity-10 bg-neutral-100 saturate-90 brightness-90 transition-filter duration-200 hover:saturate-100 hover:brightness-112"
...
function DockDivider() {
  return (
    ...
    >
      <span className="w-0.5 h-full rounded bg-border "></span>
    </motion.div>
  )
}
